...
// System
// NOTE(lvdterm): upstream uses <sys/mman.h> (mmap/munmap) and
// getpagesize() from <sys/param.h> to memory-map blocks back out of the
// temp file in at()/unmap(). Neither is available on mingw. Ported to a
// plain lseek()+read() into a heap buffer instead; see VENDORING.md.
#include <unistd.h>
#include <cstdio>
        ...
        length(0)
{
    ...
    if (blocksize == 0) {
        blocksize = ((sizeof(Block) / kPageSize) + 1) * kPageSize;
    }
...
//     }
    ...
    // Read the block's page(s) out of the backing temp file into a heap
    // buffer (upstream mmap()s the same byte range read-only instead).
    int rc = lseek(ion, static_cast<long>(j * blocksize), SEEK_SET);
    if (rc < 0) {
        perror("BlockArray::at.lseek");
        return nullptr;
    }

    auto * buffer = new unsigned char[blocksize];
    int nread = read(ion, buffer, static_cast<unsigned int>(blocksize));
    if (nread < 0) {
...
{
    delete[] reinterpret_cast<unsigned char *>(lastmap);
    lastmap = nullptr;
    lastmap_index = size_t(-1);
}
