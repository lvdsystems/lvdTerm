...
{
    // lvdterm patch: also reject selTopLeft > selBottomRight - see
    // VENDORING.md ("isSelectionValid() hardening").
    return selTopLeft >= 0 && selBottomRight >= 0 && selTopLeft <= selBottomRight;
}
        ...
        bool fullLine) const
{
    ...
    else
    {
        ...
        // lvdterm patch: clamp start to the line's actual stored length -
        // see VENDORING.md ("copyLineToStream() start clamping"). Without
        // this, a start past a short/blank line's length makes
        // qBound(0, count, length-start) below get a negative max, which
        // violates qBound's own precondition and asserts.
