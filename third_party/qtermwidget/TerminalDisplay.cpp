...
void TerminalDisplay::wheelEvent( QWheelEvent* ev )
{
  ...
  // if the terminal program is not interested in mouse events, the wheel
  // always scrolls the view itself - never send simulated Up/Down key
  // presses (lvdterm patch: upstream did this "for the benefit of
  // programs such as less" whenever _scrollBar->maximum() was 0, but that
  // is also the state of a perfectly ordinary shell prompt with less than
  // a full screen of output - e.g. right after an SSH login - where
  // readline binds Up/Down to command history, so wheel-scrolling there
  // silently overwrote the prompt with a previous command. See
  // VENDORING.md ("wheelEvent() no longer sends synthetic Up/Down keys").
  if ( _mouseMarks )
  {
    _scrollBar->event(ev);
  }
