...
#endif
#ifdef Q_OS_WIN
    // NOTE(lvdterm): no KB_LAYOUT_DIR install location on Windows (see
    // VENDORING.md); look for a kb-layouts folder shipped next to the
    // executable instead, same idea as the macOS bundle case above.
    d.setPath(QCoreApplication::applicationDirPath() + QLatin1String("/kb-layouts/"));
    if (d.exists())
        return QCoreApplication::applicationDirPath() + QLatin1String("/kb-layouts/");
#endif
...
#endif
#ifdef Q_OS_WIN
    // NOTE(lvdterm): no COLORSCHEMES_DIR install location on Windows (see
    // VENDORING.md); look for a color-schemes folder shipped next to the
    // executable instead, same idea as the macOS bundle case above.
    d.setPath(QCoreApplication::applicationDirPath() + QLatin1String("/color-schemes/"));
    if (d.exists())
    {
