...
// NOTE(lvdterm): upstream includes <sys/mman.h> here for HistoryFile::map()
// (mmap-based read cache) and CompactHistoryBlock (mmap used as a malloc
// replacement). Neither is available on mingw; both are ported to plain
// heap allocation in History.cpp/.h. See VENDORING.md.
...
public:
  ...
  CompactHistoryBlock(){
    ...
    // NOTE(lvdterm): upstream mmap()s an anonymous, private mapping here
    // purely as a malloc() replacement (see the commented-out line it
    // left behind) — there is no file backing it. malloc() is exactly
    // equivalent and portable. See VENDORING.md.
