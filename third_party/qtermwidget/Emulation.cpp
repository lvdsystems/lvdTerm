...
// NOTE(lvdterm): upstream includes "Session.h" here, but it is unused
// (see VENDORING.md) and Session.cpp/.h are not vendored.
...
void Emulation::clearScreenAndHistory()
{
    // clearEntireScreen() pushes the current screen into history rather
    // than discarding it (see Screen.cpp) - clearHistory() has to run
    // *after*, so that pushed content is wiped too, not left behind as
    // now-orphaned scrollback.
    _currentScreen->clearEntireScreen();
    _currentScreen->home();
    clearHistory();

    // Reported directly ("clear buffer seems not to refresh the view"):
    // unlike every other Screen-mutating path here, none of the above
    // goes through receiveData()/bufferedUpdate(), which is what
    // normally emits outputChanged() and drives TerminalDisplay's
    // repaint - so the screen was genuinely cleared underneath, just
    // never redrawn. showBulk() is the same call setHistory() already
    // uses to refresh immediately after an out-of-band change like this.
    showBulk();
}
