...
#include <cstdio>
#include <sys/types.h>
// NOTE(lvdterm): upstream includes <sys/mman.h> for HistoryFile::map()'s
// mmap-based read cache; ported to a heap-buffered read in map()/unmap()
// below since mingw has no <sys/mman.h>. See VENDORING.md.
#include <unistd.h>
#include <cerrno>
...
void HistoryFile::map()
{
    ...
    // Read the whole backing file into a heap buffer instead of mmap()ing
    // it read-only (mingw has no <sys/mman.h>). get() below only ever
    // reads through fileMap, so this is a drop-in replacement.
    char* buffer = new char[length];

    int rc = KDE_lseek(ion, 0, SEEK_SET);
    if (rc < 0) {
        perror("HistoryFile::map.seek");
        delete[] buffer;
        return;
    }
    rc = read(ion, buffer, length);
    if (rc < 0) {
        perror("HistoryFile::map.read");
        delete[] buffer;
        readWriteBalance = 0;
        return;
    }
