...
public:
  ...
  /**
   * lvdterm addition (see VENDORING.md - "Emulation::clearScreenAndHistory()"):
   * clears both the current screen and all scrollback, and homes the
   * cursor - a full reset of what's visible/scrollable, unlike
   * clearHistory() alone (history only, screen/cursor untouched) or
   * clearEntireScreen() alone (screen only, pushed into history rather
   * than discarded, cursor untouched).
   */
  void clearScreenAndHistory();
