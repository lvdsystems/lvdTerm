...
public:
    ...
    // Used by both the terminal's own right-click context menu and
    // MainWindow's "Terminal" menu (enabled only while a terminal pane has
    // focus) - see TerminalView.cpp's configureRequest handler and
    ...
    void startLogging(const QString &path); // no-op (silently) if the file can't be opened
    void stopLogging();
